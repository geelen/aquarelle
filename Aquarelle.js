...
(function () {
  ...
  function Aquarelle(texture, mask, options) {
    ...
    } else if (mask.nodeName && mask.src) {
      self.mask.img = mask;
      self.drawMaskImage();
    }
  ...
  Aquarelle.prototype.reset = function () {
    ...
    const Amplitude = this.transitionInRange(this.options.fromAmplitude, this.options.toAmplitude)
    this.turbulentPass.uniforms.Amplitude.value = Amplitude
    const Frequency = this.turbulentPass.uniforms.Frequency.value = this.transitionInRange(this.options.fromFrequency, this.options.toFrequency);
    const Offset = this.mask.offset = this.transitionInRange(this.options.fromOffset, this.options.toOffset);
